    ...
    while (true) {
        ...
        while (true) {
            ...
            if (prev_mail_id != email_retrieval.getMailID()) {
                str = email_retrieval.getMailContent();
                break;
            }
        }

        string response = "", filepath = "";
        handleCommand(str, response, filepath);

        const string &sender = email_retrieval.getMailSender();
        email_retrieval.respond(sender.c_str(), response.c_str(), filepath.c_str());
    }
}
